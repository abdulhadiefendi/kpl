<form action="<?= base_url('storeproduk') ?>" method="post" enctype="multipart/form-data">
  ...
  <div class="form-group">
    ...
    <select name="kategori" id="kategori" class="form-control" aria-describedby="kategorifail">
    	...
    	<?php foreach($kategori as $l){?>
    		<option value="<?= $l->idKategori ?>" <?= form_error('kategori') == $l->idKategori ? 'selected' : '' ?>><?= $l->nmKategori ?></option>
    	<?php } ?>
    </select>
  ...
  <div class="form-group">
    ...
    <select name="subkategori" id="subkategori" class="form-control" aria-describedby="subkategorifail">
    	...
    	<?php foreach($subkategori as $l){?>
    		<option value="<?= $l->idSubKategori ?>" <?= form_error('subkategori') == $l->idSubKategori ? 'selected' : '' ?>><?= $l->nmSubKategori ?></option>
    	<?php } ?>
    </select>
 ...
 <div class="row">
 	 <div class="form-group col-md-4">
	    ...
	    <input type="text" class="form-control" id="warna" aria-describedby="warnafail" placeholder="Warna" name="warna" value="<?= set_value('warna'); ?>">
	    <small id="warnafail" class="form-text text-danger"><?= form_error('warna'); ?></small>
	  </div>
	  <div class="form-group col-md-4">
	    <label for="ukuran">Ukuran</label>
	    <input type="text" class="form-control" id="ukuran" aria-describedby="ukuranfail" placeholder="Ukuran" name="ukuran" value="<?= set_value('ukuran'); ?>">
	    <small id="ukuranfail" class="form-text text-danger"><?= form_error('ukuran'); ?></small>
	  </div>
	  <div class="form-group col-md-4">
	    <label for="harga">Harga</label>
	    <input type="number" class="form-control" id="harga" aria-describedby="hargafail" placeholder="Harga" name="harga" value="<?= set_value('harga'); ?>">
 ...
 <div class="form-group">
    ...
    <input type="file" class="form-control" id="gambar" aria-describedby="gambarfail" placeholder="gambar" name="gambar" value="<?= set_value('gambar'); ?>">
    <small id="gambarfail" class="form-text text-danger"><?= form_error('gambar'); ?></small>
  </div>

  <div class="form-group">
    <label for="deskripsi">Deskripsi</label>
    <textarea name="deskripsi" id="deskripsi" cols="30" rows="7" class="form-control" aria-describedby="deskripsifail" placeholder="Deskripsi"><?= set_value('deskripsi'); ?></textarea>
