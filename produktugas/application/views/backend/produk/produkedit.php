<form action="<?= htmlspecialchars(base_url('updateproduk')) ?>" method="post" enctype="multipart/form-data">
  <div class="form-group">
    <label for="kategori">Nama Produk</label>
    <input type="text" class="form-control" id="nama" aria-describedby="namafail" placeholder="Nama Kategori" name="nama" value="<?= htmlspecialchars($d->nmProduk) ?>">
    <input type="text" class="form-control" id="id" aria-describedby="namafail" placeholder="Nama Kategori" name="id" value="<?= htmlspecialchars($d->idProduk) ?>" hidden>
  ...
  <div class="form-group">
    ...
    <select name="kategori" id="kategori" class="form-control" aria-describedby="kategorifail">
    	...
    	<?php foreach($kategori as $l){?>
    		<option value="<?= htmlspecialchars($l->idKategori) ?>" <?= $d->idKategori == $l->idKategori ? 'selected' : '' ?>><?= $l->nmKategori ?></option>
    	<?php } ?>
    </select>
 ...
 <div class="row">
 	 <div class="form-group col-md-4">
	    ...
	    <input type="text" class="form-control" id="warna" aria-describedby="warnafail" placeholder="Warna" name="warna" value="<?= htmlspecialchars($d->warna) ?>">
	    <small id="warnafail" class="form-text text-danger"><?= htmlspecialchars(form_error('warna')); ?></small>
	  </div>
	  <div class="form-group col-md-4">
	    <label for="ukuran">Ukuran</label>
	    <input type="text" class="form-control" id="ukuran" aria-describedby="ukuranfail" placeholder="Ukuran" name="ukuran" value="<?= htmlspecialchars($d->ukuran) ?>">
 ...
 <div class="form-group">
    ...
    <label for="gambar">Gambar Lama: <a target="_blank" href="<?= $d->gambar ?>"><?= htmlspecialchars($d->gambar) ?></a></label>
    <small id="gambarfail" class="form-text text-danger"><?= htmlspecialchars(form_error('gambar')); ?></small>
  </div>

  <div class="form-group">
    <label for="deskripsi">Deskripsi</label>
    <textarea name="deskripsi" id="deskripsi" cols="30" rows="7" class="form-control" aria-describedby="deskripsifail" placeholder="Deskripsi"><?= htmlspecialchars($d->deskripsi) ?></textarea>
